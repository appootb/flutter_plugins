...
public class NativeOcrPlugin: NSObject, FlutterPlugin {
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(
            name: "native_ocr", binaryMessenger: registrar.messenger())
...
private func visionCompatibleLanguageCodes(_ codes: [String]) -> [String] {
    // Vision works best with language or language-Script. Normalize common system formats like zh-Hans-CN -> zh-Hans.
    var out: [String] = []
    ...
    for raw in codes {
        ...
        if parts.count >= 2, parts[0].lowercased() == "zh" {
            // keep zh-Hans / zh-Hant, drop region.
    ...
    // De-dup, stable (case-insensitive).
    var seen = Set<String>()
    var deduped: [String] = []
    for c in out {
